...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ('Layer_start',nn.Conv2d(in_channels=3, out_channels=32, 
                                     kernel_size=4, stride=2, padding=2)), 
        ...
        self.mean_layer = nn.Linear(7*7*256 ,32) 
        self.logvar_layer = nn.Linear(7*7*256, 32) 
        self.latent_layer = nn.Linear(32, 7*7*256) 
        ...
        self.decoder = nn.Sequential(OrderedDict([
            ...
            ('DeConvUnit_3', VAEUnit(64, 32, stride=2, 
                                     padding=1, mode='decoder')),
            ('Layer_end', nn.ConvTranspose2d(in_channels=32, out_channels=3, 
                                             kernel_size=4, stride=2, padding=1)),
            ('Activation_layer', nn.Sigmoid())]))
        
    def encode(self, x):
        z = self.encoder(x)
        z = T.reshape(z, (-1, 7*7*256))
